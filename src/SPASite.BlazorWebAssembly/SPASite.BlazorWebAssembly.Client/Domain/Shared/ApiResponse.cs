...
public class ApiResponse<T> : ApiResponse
{
    ...
    public void ThrowIfInvalidOrDataNull()
    {
        base.ThrowIfInvalid();

        if (Data == null)
        {
            throw new Exception($"Unhandled validation error: Data is null");
        }
    }
}
...
public class ApiResponse
{
    ...
    public virtual void ThrowIfInvalid()
    {
        if (!IsValid)
        {
            var errors = string.Join(", ", Errors.Select(e => e.Message));
            throw new Exception($"Unhandled validation error: {errors}");
        }
    }
}
